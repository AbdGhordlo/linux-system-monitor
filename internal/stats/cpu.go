...
func parseCPUTimes(r *os.File) ([]CPUTimes, error) {
	var result []CPUTimes
	...
	for scanner.Scan() {
		...
		if len(fields) < 8 {
			continue // malformed or unexpected short line, skip defensively
		}
		...
		for _, f := range fields[1:] {
			...
			if err != nil {
				return nil, fmt.Errorf("parsing /proc/stat field %q: %w", f, err)
			}
...
// CPUPercent computes the busy percentage for each CPU line between two samples taken
// a known interval apart. prev and cur must be slices returned by ReadCPUTimes.
